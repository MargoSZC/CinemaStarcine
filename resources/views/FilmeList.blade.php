...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <div class="container">
        <h1>Listagem de Filmes {{ request()->id }}</h1>
        <form action="{{ action('App\Http\Controllers\FilmeController@search') }}" method="post">
            @csrf
            <div class="row">
                <div class="col-2">
                ...
                <div class="col-6">
                    ...
                    <a class="btn btn-dark" href="{{ action('App\Http\Controllers\FilmeController@create') }}"><i
        ...
        <table class="table table-striped table-hover">
            ...
            <tbody>
                @foreach ($filmes as $item)
                    @php
                        $nome_imagem = !empty($item->imagemfilme) ? $item->imagemfilme : 'sem_imagem.jpg';
                    @endphp
                    <tr>
                        ...
                        <td>{{ $item->horario }}</td>
                        <td>{{ $item->sinopse }}</td>
                        <td>{{ $item->categoriafilme->nome }}</td>
                        <td><img src="/storage/{{ $nome_imagem }}" width="100px" class="img-thumbnail" /> </td>
                        <td><a href="{{ action('App\Http\Controllers\FilmeController@edit', $item->id) }}"><i
                                    class='fa-solid fa-pen-to-square' style='color:orange;'></i></a></td>
                        <td>
                            <form method="POST"
                                action="{{ action('App\Http\Controllers\FilmeController@destroy', $item->id) }}">
